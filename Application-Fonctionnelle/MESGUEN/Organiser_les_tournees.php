...
<html>
	...
	<?php
		...
		foreach ($results as $ligne) {
			//On extrait chaque valeur de la ligne courante de chaque tournee
			//pour connaitre le numéro de la tournee suivante.
			$TRNNUM = $ligne[0];
			$i++;
		}
		$i+=1;
	...
	<body>
		...
		<form id='formulairetest' action ='insertinfo.php?TRNNUM=$TRNNUM' method='get'>
			...
			Chauffeur
			<?php
			//Requête SQL permettant de lister les chauffeurs par ordre alphabétique.	
			...
			Véhicule
			<?php
			//Requête SQL permettant de lister les véhicules.
			...
			if ($cpt>0) {
				...
				foreach ($results as $ligne) {
				//on extrait chaque valeur de la ligne courante pour la liste des vehicules.
					$VEHIMMAT = $ligne[0];
			...
			à <input type="text" name="Heures" id="Heures" size="2" maxlength="5" value="<?php echo $Heures;?>"/>
			<br/>
